...
import {View,Text,FlatList,SafeAreaView, Dimensions,Image,ImageBackground,StyleSheet,Platform,Alert} from 'react-native';
...
export default class Meteors extends React.Component{
    ...
    getMeteors=()=>{
        axios
        .get("https://api.nasa.gov/neo/rest/v1/feed?api_key=Np89Qdu3b274Vzcm4taYSDbqYjNl68lklcuYLPgP")
        .then((response)=>{this.setState({meteors:response.data.near_eart_objects})})
        .catch(error=>{
        ...
        <View>
            <ImageBackground source={bg_img} style={styles.backgroundImage}>
                <View styles={styles.gifContainer}>
                    ...
                    <View>
                        <Text style={[styles.cardTitle,{ marginTop:400,marginLeft}]}>{item.name}</Text>
...
keyExtractor=(item,index)=>index.toString();
    render(){
      ...
      return(
            ...
            let meteor_arr=Object_keys(this.state.meteors).map(meteor_date =>{
                return this.state.meteors[meteor_date]
            })
            let meteors=[].concat.apply([],meteor_arr);
            meteors.forEach(function(element){
                let diameter=(element.estimated_diameter.kilometers.estimated_diameter_min+element.estimated_diameter.kilometers.estimated_diameter_max)/2
                let threatScore=(diameter/element.close_approach_data[0].miss_distance_kilometers)* 1000000000
